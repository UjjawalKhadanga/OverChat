...
function Card({_id, name, owner, description, isOwner}) {
  ...
  const handleJoinClicked = async (e) => {
    e.preventDefault();
    const res = await axios.post(`${process.env.REACT_APP_API_SERVER_URL}/room/join/${_id}`, {}, {withCredentials: true})
...
function RoomConnect({userInfo}) {
    ...
    useEffect(()=>{
        axios.get(`${process.env.REACT_APP_API_SERVER_URL}/room/`, {withCredentials: true})
        .then((res)=>{
            console.log(res.data)
            setRooms(res.data)
        })
